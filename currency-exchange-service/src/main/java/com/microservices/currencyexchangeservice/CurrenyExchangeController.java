...
public class CurrenyExchangeController {
	...
	public com.microservices.currencyexchangeservice.jpa.ExchangeValue retriveExchangeValuefromH2(@PathVariable String from, 
			@PathVariable String to) {
		 com.microservices.currencyexchangeservice.jpa.ExchangeValue exchangeValue = repository.findByFromAndTo(from, to);
		 exchangeValue.setPort(Integer.parseInt(env.getProperty("local.server.port")));
		return exchangeValue;
	}
